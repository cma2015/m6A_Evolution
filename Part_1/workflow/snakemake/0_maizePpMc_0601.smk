...
rule fastp:
    ...
    output:
        creads1 = "{datadir}/{species}/01cleandata/{species}_{treat}{rep}.fastq.gz",
    ...
    shell:
        ...
        fastp -w 10 -i {input.reads1} -o {output.creads1} -j {output.json} -h {output.html}
...
rule star:
    input:
        reads1 = "{datadir}/{species}/01cleandata/{species}_{treat}{rep}.fastq.gz",
    output:
    ...
    params:
        index="../genome/zma/STAR_index_49"
    threads: 2
    shell:
        """
        set -x
